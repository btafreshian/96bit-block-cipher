  ...
  for (std::uint8_t z = 0; z < 6; ++z) {
    for (std::uint8_t y = 0; y < 4; ++y) {
      for (std::uint8_t x = 0; x < 4; ++x) {
        std::uint8_t idx = cube96::idx_of(x, y, z);
        if (idx >= cube96::kPermSize) {
          std::cerr << "idx_of out of range\n";
          return 1;
        }
        std::uint8_t rx = 0, ry = 0, rz = 0;
        cube96::xyz_of(idx, rx, ry, rz);
        if (rx != x || ry != y || rz != z) {
          std::cerr << "xyz_of(idx_of(...)) mismatch\n";
          return 1;
        }
      }
    }
